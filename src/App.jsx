...
// Initial character data - Resistencia state added
...
function App() {
  ...
  // Function to restart the game (unchanged)
  const handlePlayAgain = () => {
    logMessage("Reiniciando el juego...");
    setPlayer1Data(initialPlayer1Data); // Resets Resistencia states too
  ...
  // Helper function to resolve the 'Llave' action - Accepts bonus, removed defender Resistencia check
  const resolveLlaveAction = (attacker, defender, additionalBonus = 0) => {
    logMessage(`Resolviendo Llave [Bono Atacante: +${additionalBonus}]...`);
    // No need for setDefenderData here anymore for Resistencia check
    ...
    while (ties < 3) {
      ...
      // Attacker bonus includes base 2 + potential Resistencia bonus
      const totalAttackerBonus = 2 + additionalBonus;
      let totalDefenderBonus = 0; // Defender gets no automatic bonus here
  ...
  // Helper function to resolve a single throw in Lanzamientos Sucesivos - Accepts bonus, removed defender Resistencia check
  const resolveSingleLanzamiento = (attacker, defender, throwNumber, baseAttackerBonus = 0) => {
      ...
      // No need for setDefenderData for Resistencia check
      const baseDamage = 40;
      ...
      // No need to track usedResistenciaThisThrow

      while (ties < 3) {
          const attackerBaseRoll = rollD20();
          const defenderBaseRoll = rollD20();
          // Attacker bonus combines the throw number and the initial bonus (from Resistencia)
          const attackerRollBonus = throwNumber + baseAttackerBonus;
          let defenderRollBonus = 0; // Defender gets no automatic bonus
      ...
      if (winnerId) {
          ...
          message = `${winner.name} gana Lanzamiento #${throwNumber} (${finalAttackerRoll} vs ${finalDefenderRoll}). ${loser.name} recibe ${damageDealt} daño.`;
          logMessage(message);
      } else {
      ...
      setArenaEvent({
          ...
          winnerName: winnerId === attacker.id ? attacker.name : (winnerId === defender.id ? defender.name : 'Nadie'),
          loserName: loserId === attacker.id ? attacker.name : (loserId === defender.id ? defender.name : 'Nadie'),
          damage: damageDealt,
  ...
  // Helper function to orchestrate Lanzamientos Sucesivos - Accepts initial bonus
  const resolveLanzamientoSucesivo = async (attacker, defender, initialBonus = 0) => {
      ...
      for (let throwNumber = 1; throwNumber <= 3; throwNumber++) {
          ...
          // Pass the current state of defender
          const currentDefenderData = defender.id === player1Data.id ? player1Data : player2Data;
          // Pass the initial bonus (0 or 2) to the single throw resolver
      ...
      setArenaEvent({
          id: Date.now() + 1, type: 'action_effect', actionName: 'Fin Lanzamientos Sucesivos',
  ...
  // Function to handle the initiation of an action - UPDATED for Resistencia Choice
  const handleActionInitiate = async (actionName) => {
    ...
    const actionRequiresConcentration = ['velocidad_luz', 'salto', 'combo', 'engaño', 'lanzamientos_sucesivos'].includes(actionName);
    ...
    if (['golpe', 'lanzar_obj', 'embestir', 'cargar', 'salto', 'velocidad_luz'].includes(actionName)) {
        ...
        // Check if Resistencia bonus is available for the attacker
        if (attacker.stats.resistenciaAvailable) {
            ...
            // Resolve immediately without bonus
            logMessage(`${attacker.name} intenta una Llave normal contra ${defender.name}!`);
            const currentDefenderData = defender.id === player1Data.id ? player1Data : player2Data;
            const gameOver = resolveLlaveAction(attacker, currentDefenderData, 0); // Pass 0 bonus
        ...
        return; // Exit handleActionInitiate after handling Llave
    } else if (actionName === 'lanzamientos_sucesivos') {
        logMessage(`${attacker.name} intenta Lanzamientos Sucesivos.`);
        // Check concentration (already done above)
        // Check if used this combat
        ...
        // Check alternation (already done above)

        // Consume concentration
        setAttackerData(prev => ({ ...prev, stats: { ...prev.stats, isConcentrated: false } }));
        logMessage(`${attacker.name} usa su concentración para ${actionName.replace('_', ' ')}.`);
        // Mark as used this combat (do this *after* potential Resistencia choice)
        ...
        if (attacker.stats.resistenciaAvailable) {
            ...
            // Resolve immediately without bonus
            setAttackerData(prev => ({ ...prev, stats: { ...prev.stats, lanzamientosSucesivosUsedThisCombat: true } })); // Mark used now
            const currentDefenderData = defender.id === player1Data.id ? player1Data : player2Data;
            const gameOver = await resolveLanzamientoSucesivo(attacker, currentDefenderData, 0); // Pass 0 bonus
        ...
        return; // Exit handleActionInitiate after handling Lanzamientos
    }
    // ... (rest of the actions: presa, destrozar, romper, atrapar, concentracion, combo, engaño, fortaleza, agilidad, destreza, resistencia) remain the same
        ...
        setArenaEvent({ id: Date.now(), type: 'action_effect', actionName: 'Resistencia Activada', attackerName: attacker.name, message: `¡${attacker.name} activa Resistencia! Bono +2 a Ataque de Llave disponible.` }); // Updated message
        setActionState({ active: false, type: null, attackerId: null, defenderId: null, stage: null, allowedDefenses: null });
        const nextPlayerId = currentPlayerId === player1Data.id ? player2Data.id : player1Data.id;
        setCurrentPlayerId(nextPlayerId);
        logMessage(`Turno de ${nextPlayerId === player1Data.id ? player1Data.name : player2Data.name}`);
  ...
  // --- NEW: Handler for Resistencia Choice ---
  const handleResistenciaChoice = async (actionName, useBonus) => {
    ...
    const currentDefenderData = defender.id === player1Data.id ? player1Data : player2Data; // Get current defender state
    ...
    // Resolve the chosen action with the determined bonus
    if (actionName === 'llave') {
        gameOver = resolveLlaveAction(attacker, currentDefenderData, bonusValue);
        // Mark Llave used for alternation *after* resolution
        if (!gameOver) {
             setAttackerData(prev => ({ ...prev, stats: { ...prev.stats, lastActionType: 'llave' } }));
        }
    } else if (actionName === 'lanzamientos_sucesivos') {
        // Mark Lanzamientos used *before* starting the sequence if Resistencia is involved
        ...
        // Mark Lanzamientos used for alternation *after* resolution
        if (!gameOver) {
            setAttackerData(prev => ({ ...prev, stats: { ...prev.stats, lastActionType: 'lanzamientos_sucesivos' } }));
        }
    }

    // End turn if game is not over
    if (!gameOver && actionState.stage !== 'game_over') {
        setActionState({ active: false, type: null, attackerId: null, defenderId: null, stage: null, allowedDefenses: null });
        const nextPlayerId = currentPlayerId === player1Data.id ? player2Data.id : player1Data.id;
        setCurrentPlayerId(nextPlayerId);
  ...
  // --- handleDefenseSelection (unchanged) ---
  const handleDefenseSelection = async (defenseType) => {
    ...
    const actionKey = actionType.toLowerCase().replace('_opcion', '_op');
    ...
    if (actionType === 'Engaño') {
        ...
        if (roll === 1) { defenseSuccessful = false; rollOutcome = 'failure'; damageToDefender = baseDamage; }
            ...
            let [min, max] = defender.defenseRanges.contraatacar; let bonus = usingDestreza ? 2 : 0;
            targetMin = Math.max(1, min - bonus); targetMax = max; // Counter bonus lowers target
            if (roll >= targetMin && roll <= targetMax) { defenseSuccessful = true; damageToAttacker = Math.floor(defender.actions.golpe / 2); rollOutcome = 'countered'; } else { damageToDefender = baseDamage; rollOutcome = 'failure'; }
        }
        ...
        if (defenseSuccessful || gameOver || currentHit >= 3) {
            ...
            logMessage(`¡El golpe #${currentHit} conecta! ${attacker.name} continúa dengan golpe #${nextHit}...`);
        ...
        if (roll === 1) { defenseSuccessful = false; rollOutcome = 'failure'; damageToDefender = baseDamage; }
            ...
            else if (actionType === 'Atrapar_Opcion2' && !usingFortaleza && defenseType !== 'bloquear') { rollOutcome = 'invalid'; }
            ...
            if (rollOutcome !== 'invalid') {
             ...
             let [min, max] = defender.defenseRanges.contraatacar; let bonus = 0;
  ...
  const handleAtraparFollowupSelect = async (optionId) => {
    ...
    switch (optionId) {
      ...
      case 'atrapar_op6': {
        ...
        setArenaEvent({ id: Date.now(), type: 'action_effect', actionName: 'Romper Mejorado', message: `${attacker.name} intenta romper una parte del cuerpo dengan bono de +4.` });
        break;
      }
      case 'atrapar_op7': {
  ...
  // --- Render Logic --- (Pass handleResistenciaChoice to PlayerArea)
  return (
